...
export default function Login() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      else { await signUp(form.email, form.password, form.name); toast.success('Account created. Check your email if confirmation is enabled.'); setMode('login') }
    } catch (e) { toast.error(e.message) } finally { setBusy(false) }
  }
  return <div className="auth-page">
    <div className="auth-card">
      <div className="brand large"><div className="brand-mark">R</div><div><strong>Rentuki</strong><small>Rental management</small></div></div>
      <h1>{mode === 'login' ? 'Welcome back' : 'Create your account'}</h1>
      <p className="muted">Securely manage your rental operations.</p>
